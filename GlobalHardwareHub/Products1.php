...
<head>
    ...
    <title>Global Hardware Hub | Products Page</title>
    <!-- Font Awesome 6 -->
...
<body>
    ...
    <header class="header">
        <div class="nav-container">
            ...
            <ul class="nav-links" id="desktopNav">
                ...
                <li class="nav-item"><a href="CompareProducts.php" class="nav-link"> Compare Products</a></li>
    ...
    <div class="products-layout">
        <!-- Filter Sidebar - WITH EXACT DATABASE CATEGORY NAMES -->
        <aside class="filter-sidebar" data-aos="fade-right" data-aos-duration="600" data-aos-offset="100">
            ...
            <div class="filter-group">
                <label>PKR Price Range</label>
                <div class="price-range">
                    <input type="number" id="filterMinPrice" class="filter-input" placeholder="Min PKR" step="1000" min="0">
                    <input type="number" id="filterMaxPrice" class="filter-input" placeholder="Max PKR" step="1000" min="0">
                </div>
            </div>

            <div class="filter-buttons">
                <button id="applyFilterBtn" class="btn-apply"><i class="fas fa-check"></i> Apply Filters</button>
        ...
        <main class="products-main">
            ...
            <h1 class="page-title"><i class="fas fa-microchip"></i> All Products <span id="activeFilterBadge" class="active-filters-badge"></span></h1>
            <!-- Products Grid -->
    ...
    <script>
        ...
        function applyUrlParamsToFilters() {
            ...
            if (urlParams.category) {
                ...
                if (categorySelect) {
                    for (let i = 0; i < categorySelect.options.length; i++) {
                        if (categorySelect.options[i].value === urlParams.category) {
            ...
            if (urlParams.brand) {
                ...
                if (brandSelect) {
                    for (let i = 0; i < brandSelect.options.length; i++) {
                        if (brandSelect.options[i].value.toLowerCase() === urlParams.brand.toLowerCase() ||
                            brandSelect.options[i].value === urlParams.brand) {
            ...
            if (urlParams.minPrice) {
                ...
                if (minPriceInput) minPriceInput.value = urlParams.minPrice;
            }
            
            if (urlParams.maxPrice) {
                const maxPriceInput = document.getElementById('filterMaxPrice');
                if (maxPriceInput) maxPriceInput.value = urlParams.maxPrice;
            }
            
            // Update active filter badge
            updateActiveFilterBadge(urlParams);
        }
        
        function updateActiveFilterBadge(urlParams) {
            const badge = document.getElementById('activeFilterBadge');
            if (!badge) return;
            
            const activeFilters = [];
            if (urlParams.category) {
                const displayName = getCategoryDisplayName(urlParams.category);
                activeFilters.push(displayName);
            }
            if (urlParams.brand) activeFilters.push(urlParams.brand);
            if (urlParams.minPrice) activeFilters.push(`Min: PKR ${urlParams.minPrice}`);
            if (urlParams.maxPrice) activeFilters.push(`Max: PKR ${urlParams.maxPrice}`);
            
            if (activeFilters.length > 0) {
                badge.textContent = `Filtered: ${activeFilters.join(' | ')}`;
        ...
        async function addToCart(product) {
            ...
            if (!isCustomerRole) {
                showToastMessage("Only customers can add items to cart.", 'error');
                return false;
            }
        ...
        function renderMobileMenu() {
            ...
            menuItems.forEach(item => {
                html += `<div class="mobile-nav-item"><a href="${item.link}" style="display:block; padding:0.8rem 0; text-decoration:none; color:#374151;">${item.title}</a></div>`;
            });
            ...
            const productsHTML = products.map((product, index) => {
                ...
                return `
                    <div class="product-card" data-product-id="${product.product_id}" data-aos="fade-up" data-aos-duration="400" data-aos-delay="${(index % 12) * 50}">
                        ...
                        <div class="product-category"><i class="fas fa-folder"></i> ${escapeHtml(getCategoryDisplayName(product.category || 'Hardware'))}</div>
                        <div class="product-price">PKR ${typeof product.price === 'number' ? product.price.toFixed(2) : product.price}</div>
                        <div class="product-actions">
                            <button class="btn-cart" onclick="addToCart({id:${product.product_id}, name:'${escapedName}', price:${product.price}, image:'${imageUrl}'})"><i class="fas fa-cart-plus"></i> Cart</button>
                            ...
                            <button class="btn-details" onclick="viewDetails(${product.product_id})"><i class="fas fa-eye"></i> View</button>
                        </div>
                    </div>
                `;
            }).join('');
        ...
        function updateUrlWithFilters() {
            ...
            const newUrl = params.length > 0 ? `Products1.php?${params.join('&')}` : 'Products1.php';
        ...
        function resetFilters() {
            ...
            window.history.pushState({}, '', 'Products1.php');
            const badge = document.getElementById('activeFilterBadge');
            badge.textContent = '';
            badge.style.display = 'none';
        }
        ...
        async function init() {
            ...
            // Apply URL parameters to filter inputs before loading products
