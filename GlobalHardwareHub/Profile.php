...
<head>
    ...
    <title>Global Hardware Hub | My Profile</title>
    <!-- Font Awesome 6 -->
    ...
    <style>
        * {
        ...
        /* Cards - White */
        /* A. Profile Card Hover Animation */
        .profile-card {
            background: #FFFFFF;
            border-radius: 32px;
            padding: 2rem;
        ...
        /* D. Profile Image Zoom Animation */
        .profile-pic:hover {
            transform: scale(1.05);
        }
...
<body>
    ...
    <div class="profile-container">
        ...
        <h1 class="page-title" data-aos="fade-up" data-aos-duration="600" data-aos-offset="50"><i class="fas fa-user-circle"></i> My Profile</h1>

        <!-- H. Scroll Reveal - Personal Information Card -->
        <div class="profile-card" data-aos="fade-up" data-aos-duration="600" data-aos-offset="50" data-aos-delay="50">
            <h2><i class="fas fa-pen-alt" style="color: var(--primary);"></i> Personal Information</h2>
            <div id="personalSuccessMsg" class="success-msg"></div>

            <div class="profile-pic-section">
                <img id="profilePic" class="profile-pic" src="uploads/profile/default-avatar.png" alt="Profile Picture">
                <input type="file" id="profilePicUpload" accept="image/jpeg,image/png,image/jpg" style="display:none">
                <button type="button" class="pic-upload-btn" id="uploadPicBtn"><i class="fas fa-camera"></i> Upload
                    Photo</button>
            </div>
            ...
            <button class="btn-primary" id="savePersonalBtn"><i class="fas fa-save"></i> Save Changes</button>
        </div>

        <!-- H. Scroll Reveal - Change Password Card -->
        <div class="profile-card" data-aos="fade-up" data-aos-duration="600" data-aos-offset="50" data-aos-delay="100">
            <h2><i class="fas fa-lock" style="color: var(--primary);"></i> Change Password</h2>
            <div id="passwordSuccessMsg" class="success-msg"></div>

            <div class="form-group">
                <label><i class="fas fa-key"></i> Current Password</label>
                <div class="password-wrapper">
                    <input type="password" id="currentPassword" placeholder="Enter current password">
                    <button type="button" class="toggle-password" data-target="currentPassword"><i
            ...
            <div class="form-group">
                ...
                <div class="password-wrapper">
                    ...
                    <button type="button" class="toggle-password" data-target="newPassword"><i
                            class="fas fa-eye"></i></button>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="password-wrapper">
                    ...
                    <button type="button" class="toggle-password" data-target="confirmPassword"><i
                            class="fas fa-eye"></i></button>
                </div>
            </div>
            <button class="btn-primary" id="savePasswordBtn"><i class="fas fa-sync-alt"></i> Update Password</button>
        </div>
    </div>
    ...
    <script>
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (profile.profile_image && profile.profile_image !== 'null') {
                        document.getElementById('profilePic').src = profile.profile_image;
                    } else {
                        document.getElementById('profilePic').src = 'uploads/profile/default-avatar.png';
                    }
        ...
        async function updateProfile(fullName, email, phone, dateOfBirth) {
        ...
        async function changePassword(currentPassword, newPassword, confirmPassword) {
            ...
            try {
                const response = await fetch('change_user_password.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: formData
                });
        ...
        function showPersonalSuccess(message) {
            const msgDiv = document.getElementById('personalSuccessMsg');
        ...
        function showPasswordSuccess(message) {
            const msgDiv = document.getElementById('passwordSuccessMsg');
            msgDiv.innerHTML = `<i class="fas fa-check-circle"></i> ${message}`;
            msgDiv.classList.add('show');
            setTimeout(() => msgDiv.classList.remove('show'), 3000);
        }
        ...
        function checkPasswordStrength(pwd) {
            ...
            if (pwd.length === 0) return { level: 0, text: "Weak", color: "#E5E7EB" };
        ...
        document.getElementById('savePersonalBtn').addEventListener('click', async function () {
            ...
            const result = await updateProfile(fullName, email, phone, dob);

            if (result.success) {
                showPersonalSuccess('Personal information updated successfully!');
                await loadProfile();
            } else {
        ...
        document.getElementById('profilePicUpload').addEventListener('change', async function (e) {
            ...
            if (result.success) {
                document.getElementById('profilePic').src = result.image_url + '?t=' + new Date().getTime();
                showPersonalSuccess('Profile image updated successfully!');
            } else {
                alert('Error: ' + (result.message || 'Failed to upload image'));
            }
        ...
        document.getElementById('newPassword').addEventListener('input', function () {
            ...
            document.getElementById('strengthFill').style.width = result.level + '%';
            document.getElementById('strengthFill').style.backgroundColor = result.color;
            document.getElementById('strengthText').innerText = result.text;
            document.getElementById('strengthText').style.color = result.color;
        });
        ...
        document.getElementById('savePasswordBtn').addEventListener('click', async function () {
            ...
            const result = await changePassword(currentPwd, newPwd, confirmPwd);
