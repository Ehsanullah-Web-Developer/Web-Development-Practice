...
<head>
    ...
    <title>Global Hardware Hub | All Vendors</title>
    <!-- Font Awesome 6 -->
    ...
    <style>
        * {
        ...
        .btn-filter::before, .btn-refresh::before {
            content: '';
            position: absolute;
            top: 0;
            left: -100%;
        ...
        .btn-filter:hover::before, .btn-refresh:hover::before {
            left: 100%;
        }
        ...
        .vendor-card:hover .vendor-info h3 {
            color: var(--primary);
        }
        ...
        .vendor-grid .vendor-card:nth-child(9) { animation-delay: 0.45s; }
        .vendor-grid .vendor-card:nth-child(10) { animation-delay: 0.5s; }
...
<body>
    ...
    <div class="vendors-container">
        <div class="breadcrumb">
            <a href="FYPHome.php"><i class="fas fa-home"></i> Home</a> / <span>Vendors</span>
        </div>
        <h1 class="page-title"><i class="fas fa-store"></i> All Vendors</h1>

        <div class="filter-section" data-aos="fade-up" data-aos-duration="800">
            ...
            <div class="filter-row">
                <div class="rating-filter">
                    ...
                    <button class="rating-btn" data-rating="2">2★ & up</button>
                </div>
    ...
    <script>
        ...
        function showMessage(message, isError = false) {
            ...
            if (container) {
                container.innerHTML = `<div class="empty-state"><h3><i class="fas ${isError ? 'fa-exclamation-circle' : 'fa-info-circle'}"></i> ${message}</h3></div>`;
            }
        }
        ...
        function showLoading() {
            const container = document.getElementById("vendorsContainer");
            if (container) {
                container.innerHTML = `<div class="empty-state"><h3><i class="fas fa-spinner fa-pulse"></i> Loading vendors...</h3></div>`;
            }
        }
        ...
        async function fetchVendors() {
            ...
            try {
                ...
                if (Array.isArray(data)) {
                    ...
                    showMessage('Invalid data format', true);
                }
            } catch (error) {
                console.error('Fetch error:', error);
                showMessage('Failed to load vendors. Please refresh the page.', true);
            }
        }
        ...
        function filterVendors() {
            ...
            if (filters.search) {
                filtered = filtered.filter(v =>
                    v.store_name && v.store_name.toLowerCase().includes(filters.search.toLowerCase())
                );
            }
            ...
            if (filters.sort === "rating") {
                ...
                filtered.sort((a, b) => (b.total_products || 0) - (a.total_products || 0));
        ...
        function renderVendors() {
            ...
            if (filtered.length === 0) {
                container.innerHTML = `<div class="empty-state"><h3><i class="fas fa-search"></i> No vendors found</h3><p style="color:#6B7280;">Try adjusting your filters</p></div>`;
                return;
            }

            if (currentView === "grid") {
                container.className = "vendor-grid";
                container.innerHTML = filtered.map((vendor, index) => `
                    <div class="vendor-card" data-aos="fade-up" data-aos-delay="${index * 50}" data-aos-duration="600">
                        <div class="vendor-header">
                            <img class="vendor-logo" src="${vendor.logo_url || 'https://placehold.co/70x70/2563eb/white?text=Vendor'}" alt="${escapeHtml(vendor.store_name)}" onerror="this.src='https://placehold.co/70x70/2563eb/white?text=Vendor'">
                            <div class="vendor-info">
                                <h3>${escapeHtml(vendor.store_name)}</h3>
                                <div class="stars">${renderStars(vendor.rating || 0)} (${vendor.rating || 'N/A'})</div>
                                <div class="vendor-meta"><i class="fas fa-star"></i> ${vendor.rating || 'New'} star rating</div>
                            </div>
                        </div>
                        <div class="vendor-stats">
                            <i class="fas fa-store"></i> ${escapeHtml(vendor.description || 'No description available').substring(0, 80)}${(vendor.description || '').length > 80 ? '...' : ''}
                        </div>
                        <div class="vendor-actions">
                            <button class="btn-visit" onclick="visitStore(${vendor.vendor_id}, '${escapeHtml(vendor.store_name).replace(/'/g, "\\'")}')"><i class="fas fa-store"></i> Visit Store →</button>
                        </div>
                    </div>
                `).join('');
            } else {
                ...
                container.innerHTML = filtered.map((vendor, index) => `
                    <div class="vendor-card" data-aos="fade-left" data-aos-delay="${index * 50}" data-aos-duration="600">
                        <img class="vendor-logo" src="${vendor.logo_url || 'https://placehold.co/80x80/2563eb/white?text=Vendor'}" alt="${escapeHtml(vendor.store_name)}" onerror="this.src='https://placehold.co/80x80/2563eb/white?text=Vendor'">
                        <div class="vendor-info">
                            <h3>${escapeHtml(vendor.store_name)}</h3>
                            <div class="stars">${renderStars(vendor.rating || 0)} (${vendor.rating || 'N/A'})</div>
                            <div class="vendor-meta"><i class="fas fa-star"></i> ${vendor.rating || 'New'} star rating</div>
                            <div class="vendor-description" style="font-size:0.75rem; color:#6B7280; margin-top:0.5rem;">${escapeHtml(vendor.description || 'No description available')}</div>
                        </div>
                        <div class="vendor-actions">
                            <button class="btn-visit" onclick="visitStore(${vendor.vendor_id}, '${escapeHtml(vendor.store_name).replace(/'/g, "\\'")}')"><i class="fas fa-store"></i> Visit Store →</button>
                        </div>
        ...
        function applyFilters() {
            ...
            filters.sort = sortFilter ? sortFilter.value : "rating";
